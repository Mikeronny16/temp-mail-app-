  ...
  return (
    <section id="menu" ref={ref} style={{ padding: '7rem 6rem', background: 'var(--surface)' }}>
      <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}
        ...
        <h2 className="display" style={{ fontSize: 'clamp(2.2rem, 4vw, 3.5rem)', fontWeight: 700, lineHeight: 1.15, letterSpacing: '-0.02em' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.2 }}
        ...
        {cats.map(cat => (
          <button key={cat} onClick={() => setActive(cat)} style={{
            ...
            fontSize: '0.8rem', fontWeight: active === cat ? 600 : 400,
