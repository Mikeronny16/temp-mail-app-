    ...
    <section style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8rem 4rem 4rem 6rem', position: 'relative' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.55 }}
          ...
          <a href="#menu" style={{ background: 'var(--gold)', color: '#0C1A14', padding: '0.9rem 2.25rem', borderRadius: '999px', fontSize: '0.875rem', fontWeight: 700, textDecoration: 'none', letterSpacing: '0.04em', transition: 'opacity 0.2s' }}
            onMouseEnter={e => (e.currentTarget.style.opacity = '0.88')}
            onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>
            View Menu
          </a>
          <a href="#location" style={{ color: 'var(--text-muted)', fontSize: '0.875rem', fontWeight: 500, textDecoration: 'none', borderBottom: '1px solid var(--border)', paddingBottom: '2px', transition: 'color 0.2s' }}
