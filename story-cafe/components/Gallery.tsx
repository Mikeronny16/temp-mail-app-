    ...
    <section id="gallery" className="section-pad" style={{ background: 'var(--surface)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }} style={{ marginBottom: '2.5rem', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <div style={{ fontSize: '0.72rem', color: 'var(--terra)', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Instagram</div>
            <h2 className="display" style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.6rem)', color: 'var(--text)', fontStyle: 'italic' }}>
          ...
          <a href="https://www.instagram.com/thestorycafe.88" target="_blank" rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--terra)', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 600, border: '1.5px solid var(--border)', borderRadius: '999px', padding: '0.45rem 1.1rem' }}>
        ...
        <div className="gal-grid">
          {photos.map((p, i) => (
            <motion.div key={i}
              ...
              style={{ borderRadius: '0.9rem', overflow: 'hidden', aspectRatio: '1', position: 'relative', boxShadow: '0 4px 20px rgba(28,17,8,0.10)', cursor: 'pointer', transition: 'all 0.25s' }}
            >
              <div style={{ position: 'absolute', inset: 0, background: `url("${p.url}") center/cover` }} />
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(28,17,8,0.45) 0%, transparent 55%)', opacity: 0, transition: 'opacity 0.25s' }}
                onMouseEnter={e => { e.currentTarget.style.opacity = '1' }}
                onMouseLeave={e => { e.currentTarget.style.opacity = '0' }}
              />
              <div style={{ position: 'absolute', bottom: '0.75rem', left: '0.75rem', fontSize: '0.7rem', color: '#fff', fontWeight: 600, background: 'rgba(28,17,8,0.4)', backdropFilter: 'blur(6px)', padding: '0.2rem 0.55rem', borderRadius: '999px' }}>{p.tag}</div>
            </motion.div>
          ))}
        </div>
      </div>
